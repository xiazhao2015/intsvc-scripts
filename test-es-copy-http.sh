...
get_running_pod() {
    ...
    #* Added end
    oc get pods -l $lblname=$1 $3| awk -v sel=$1 '$1 ~ sel && $3 == "Running" {print $1}'
}
...
write_and_verify_logs() {
    ...
    fpod=`get_running_pod frontend name "-n test" | head -1`
    if [ -z "$fpod" ] ; then
    ...
    logmessage=`uuidgen`
    #* updated by chunchen@redhat.com
    oc exec -n test $fpod -- curl --connect-timeout 1 -s "http://$testip:$testport/$logmessage" > /dev/null 2>&1 || echo will generate a 404
